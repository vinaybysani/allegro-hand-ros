...
void controlAllegroHand::init(int mode) {
  ...
  if (!ret) {
    ROS_INFO("CAN: Version info: %s", txt);
  } else {
...
void controlAllegroHand::_writeDeviceMsg(DWORD command, DWORD from, DWORD to, BYTE len,
                                         unsigned char *data) {
  TPCANMsg msg1;
  DWORD Txid;
  ...
  switch (cmd) {
    case ID_CMD_QUERY_CONTROL_DATA:
      if (src >= ID_DEVICE_SUB_01 && src <= ID_DEVICE_SUB_04) {
        ...
        values[0] = ((double) mEncoderDirection[lIndexBase + 0] * (double) tmppos[0] - 32768.0 -
                     (double) mEncoderOffset[lIndexBase + 0]) * (333.3 / 65536.0) * (M_PI / 180.0);
        values[1] = ((double) mEncoderDirection[lIndexBase + 1] * (double) tmppos[1] - 32768.0 -
                     (double) mEncoderOffset[lIndexBase + 1]) * (333.3 / 65536.0) * (M_PI / 180.0);
        values[2] = ((double) mEncoderDirection[lIndexBase + 2] * (double) tmppos[2] - 32768.0 -
                     (double) mEncoderOffset[lIndexBase + 2]) * (333.3 / 65536.0) * (M_PI / 180.0);
        values[3] = ((double) mEncoderDirection[lIndexBase + 3] * (double) tmppos[3] - 32768.0 -
                     (double) mEncoderOffset[lIndexBase + 3]) * (333.3 / 65536.0) * (M_PI / 180.0);
